...
class BaseReal:
    ...
    def break_talk(self):
        self.tts.break_talk()
        self.asr.flush_talk()
        logger.info(f"AVATAR STATE just now is: TTS = {self.tts.state}")

    def flush_talk(self):
        logger.info(f"AVATAR STATE was: TTS = {self.tts.state}")
        self.tts.flush_talk()
        self.asr.flush_talk()
        logger.info(f"AVATAR STATE just now: TTS = {self.tts.state}")
    ...
    def get_audio_stream(self,audiotype):
        ...
        if self.custom_audio_index[audiotype]>=self.custom_audio_cycle[audiotype].shape[0]:
            self.curr_state = 1  #Текущее видео не зацикливается и переключается в режим без звука.
        return stream
